 ...
 */
export async function resolveShadowAccounts(params) {
    ...
    for (let from = start; from < end; from += MAX_SCAN_RANGE) {
        ...
        const window = (await anonymizer.get_shadow_accounts(partialCommitment, from, to, untilUndeployed));
        infos.push(...window);
        if (untilUndeployed && window.length < to - from)
            break;
    }
    return infos;
}
