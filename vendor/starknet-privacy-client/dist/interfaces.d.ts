import type { BigNumberish, Call, CallDetails, EstimateFeeResponseOverhead, ProviderInterface, STRK20_ACTION, STRK20_CALL_AND_PROOF, STRK20_CALLDATA_ITEM, STRK20_PROOF, UniversalDetails } from "starknet";
 ...
 */
export type Strk20Action = STRK20_ACTION | STRK20_COMPUTE_AND_INVOKE_ACTION | STRK20_SHADOW_ACCOUNT_INVOKE_ACTION;
/**
...
export interface PrivacyWallet {
    ...
    executeWithProof(calls: Call[], proof?: STRK20_PROOF): Promise<{
        transaction_hash: string;
    }>;
    /** Estimate the fee for `calls` on the node — the wallet is the account, so no sender is passed. */
    estimateInvokeFee(calls: Call[], details?: UniversalDetails): Promise<EstimateFeeResponseOverhead>;
}
/**
...
export interface PrivacyClient {
     ...
     */
    submit(actions: Strk20Action[], options?: SubmitOptions & {
        simulate?: false;
...
export type PrivacyComputeInvokeCallBuilder = (args: PrivacyInvokeArgs) => PrivacyComputeInvokeDetails;
/**
...
export interface ShadowAccountsBuilder {
    ...
    invoke(nonce: BigNumberish, options: {
        calls: Call[];
        collectPolicy?: Strk20CollectPolicy;
    }): PrivacyBuilder;
}
