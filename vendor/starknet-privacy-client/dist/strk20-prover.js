 ...
 */
export class CorePrivateTransfersProver {
    transfers;
    node;
    storage;
    constructor(config) {
    ...
    async prove(actions, simulate = false) {
...
function translate(builder, actions) {
        ...
        switch (action.type) {
            ...
            case "shadow_account_invoke":
                ...
                builder.shadowAccounts(action.dapp_name).invoke(action.nonce, {
                    ...
                    collectPolicy: action.collect_policy.type === "exact"
                        ? { type: "exact", amount: num.toBigInt(action.collect_policy.amount) }
                        : action.collect_policy,
                });
                break;
            default:
                throw new Error(`unsupported strk20 action: ${safeStringify(action)}`);
        }
    }
}
...
function substitute(item, args) {
    ...
    if (openNote) {
        ...
        if (!note) {
            throw new Error(`${item} is out of range: this transaction creates ${args.openNotes.length} open note(s)`);
        }
        return num.toHex(note.noteId);
    }
    if (item === "${poolAddress}")
        return num.toHex(args.poolAddress);
    return num.toHex(num.toBigInt(item));
}
/** Map a core `CallAndProof` to the strk20 RPC shape (snake_case call fields, `proof_facts`). */
function toStrk20CallAndProof(result) {
    return {
...
//# sourceMappingURL=strk20-prover.js.map